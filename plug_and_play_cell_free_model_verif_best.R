...
# More precisely, it is the script showing the chosen TF/reporter concentrations given the best behavior despite resource competition.
...
global_parameters = c(
        ...
        lambda_TF = 0.0016,
        n_copy_GFP = 100,  # Will be changed in the loop
        n_copy_TF = 30,  # Will be changed in the loop
        omega_GFP = 4, 
        omega_TF = 5, 
        benzoic_kd = 100,
        activated_kd = 50,
...
solver_all_resource <- function(t, state, pars) {
  with (as.list(c(state, pars)),{
    ...
    # Rate is expressed In µM. * 0.001 since enzyme is in nM. Floor is to ensure at least 1 enzyme is produced
...
TF_new_exp = c(10, 30, 100)
GFP_new_exp = c(30, 100)

# For various Tf and GFP concentrations, visualise the heatmap data.

for (TF_testing_concentration in TF_new_exp) {
  for (GFP_testing_concentration in GFP_new_exp) {
    all_parameters["n_copy_GFP"] = GFP_testing_concentration
    all_parameters["n_copy_TF"] = TF_testing_concentration
    
    all_parameters["n_copy_enz"] = 0
    init_solver["inducer"] = 0
    ...
    name_for_saving = paste("TF", TF_testing_concentration, 'GFP', GFP_testing_concentration, sep = '_')
    folder_for_concentration_images_strategy = "C:/Users/Mathilde/Desktop/Peter_final_versions/version_2/images_best"
    colnames(inducer_enzyme_adder_df) = colnames_for_visu
    # Heatmap values can be saved by uncommenting the following line
    write.csv(inducer_enzyme_adder_df, paste(folder_for_concentration_images_strategy, '/', name, name_for_saving,'.csv', sep = ''))
    plot_2D_heatmap(df_means= inducer_enzyme_adder_df, df_sd = NULL, title_base = "Visualising adder 2D heatmap", saving = TRUE, name_for_saving = name_for_saving, folder_for_concentration_images_strategy = folder_for_concentration_images_strategy)
  }
}
